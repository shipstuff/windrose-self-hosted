...
- **Empty-server idle CPU is a known upstream bug, but it's quarantinable.** With no players connected, two threads named `GameThread` busy-spin in userspace, burning ~1.82 cores on dispatch that has no network peer to pace against. `strace` on the hot threads shows zero syscalls for 5+ seconds — it's pure spin-wait, unreachable from Engine.ini / ConsoleVariables.ini / launch args (15 plausible knobs tested, all no-ops — see `memory/windrose_idle_cpu_known_bug.md`). The Shipping binary strips most of the relevant CVars out of the build entirely.

  **The mitigation:** `WINE_CPU_TOPOLOGY=1:0` tells Wine to advertise a single-core topology to the Windows process, which pins the spin to one core and leaves the other(s) free for the P2P handshake burst. Idle host load drops from 195% to ~99%, UI-sidecar latency halves. It's a quarantine, not a fix — the bug still wastes one full core — but it's the difference between "handshake starves" and "1 player plays fine on 2 vCPU." `bare-linux/install.sh` auto-enables it on ≤2 vCPU hosts; the Helm chart exposes `wineCpuTopology` (leave empty on ≥3 vCPU); Docker Compose reads `WINE_CPU_TOPOLOGY` from the env.

  Sizing floors (validated on DigitalOcean bare-Linux droplets 2026-04-18/19):
  - **1 vCPU / any RAM**: unplayable. Even with the mitigation there's no second core for the handshake. Only an upstream fix changes this.
  - **2 vCPU / 2 GB**: unplayable for a different reason — a delayed ~500 MiB allocation at t≈9 min crashes a 2 GB host's page reclamation regardless of swap.
  - **2 vCPU / 4 GB + mitigation**: works. Handshake completes, 1 player plays fine.
  - **≥3 vCPU / 4 GB**: comfortable without the mitigation; leave it off so the game can use extra compute during peaks.

  Upstream fix tracker: [Steam community thread](https://steamcommunity.com/app/3041230/discussions/0/807974232125564069/).
