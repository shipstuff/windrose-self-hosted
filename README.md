...
- **Empty-server idle CPU is a known upstream bug (but we now have a binary-patch workaround).** With no players connected, two threads named `GameThread` busy-spin in userspace, burning ~1.82 cores on dispatch that has no network peer to pace against. `strace` on the hot threads shows zero syscalls for 5+ seconds — pure spin-wait, unreachable from Engine.ini / ConsoleVariables.ini / launch args / Proton env vars (15+ knobs tested, all no-ops — see `memory/windrose_idle_cpu_known_bug.md`). Round-3 reverse-engineering identified the hot function as `boost::asio::detail::socket_select_interrupter::reset()` (string constant `asio\detail\impl\socket_select_interrupter.ipp` embedded in the function body). A 43-byte binary patch at `tools/patch-idle-cpu.py` injects a `Sleep(1)` call into the loop-continue tail, dropping idle CPU from ~206% to ~5% (97.5% reduction, verified on the canary pod 2026-04-19). Opt in via `WINDROSE_PATCH_IDLE_CPU=1` (Helm: `patchIdleCpu: "1"`; compose: `WINDROSE_PATCH_IDLE_CPU=1`). The patch is md5-gated to a specific Windrose build and a no-op on unknown builds; `--revert` flag on the script undoes the change. See `memory/windrose_idle_cpu_patch_works.md` for the full derivation, or pass the Steam community thread URL ([here](https://steamcommunity.com/app/3041230/discussions/0/807974232125564069/)) along with the patch notes to the Windrose devs if you're comfortable doing so. **The patch is offered as-is with no warranty and no guarantees of correctness, stability, or future applicability.** Operators run it against their own SteamCMD-pulled copy of the binary — this project does not distribute or bake the patched executable into any image or release. Applying a binary patch may also conflict with the Windrose EULA or Steam Subscriber Agreement; review the terms that apply to you before enabling, and accept that the full risk — functional, legal, and otherwise — rests with you.
