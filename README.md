...
- **Empty-server idle CPU is a known upstream bug.** With no players connected, two threads named `GameThread` busy-spin in userspace, burning ~1.82 cores on dispatch that has no network peer to pace against. `strace` on the hot threads shows zero syscalls for 5+ seconds — pure spin-wait, unreachable from Engine.ini / ConsoleVariables.ini / launch args / Proton env vars (15+ knobs tested, all no-ops — see `memory/windrose_idle_cpu_known_bug.md`). The Shipping binary strips most of the relevant CVars out of the build entirely. Only the upstream devs can fix this — tracker: [Steam community thread](https://steamcommunity.com/app/3041230/discussions/0/807974232125564069/).
  ...
  - **2 vCPU / 4 GB**: works with no CPU cap. First-connect is slow because terrain generation on a fresh world competes with the idle bug for CPU; on slower shared vCPUs the first connect can occasionally race Coturn's ~180 s timeout. Pre-generated / imported saves skip the spike. **Don't add a CPU cap on this tier** — it makes the handshake worse, not better.
